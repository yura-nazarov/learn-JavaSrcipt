...
document.querySelector('.b-6').onclick = () => {
   ...
   document.querySelector('.out-6').innerHTML = f6(a6);
};
...
document.querySelector('.b-7').onclick = () => {
   ...
   document.querySelector('.out-7').innerHTML = f7(a7);
};
...
function f8(arr) {
   ...
   let out = 0;
   for (let key in arr) {
      if (key === i8) out = arr[key];
   }
   return out;
}

document.querySelector('.b-8').onclick = () => {
   document.querySelector('.out-8').innerHTML = f8(a8);
};
...
function f9(arr) {
   ...
   for (let key in arr) {
      if (arr[key] == i9) out += key + ' ';
   }
   return out;
}

document.querySelector('.b-9').onclick = () => {
   document.querySelector('.out-9').innerHTML = f9(a9);
};
...
document.querySelector('.b-10').onclick = () => {
   ...
   document.querySelector('.out-10').innerHTML = f10(a10, 222);
};
...
document.querySelector('.b-11').onclick = () => {
   document.querySelector('.out-11').innerHTML = f11(a11);
}
...
function f13() {
   ...
   let out = 0;
   for (let key in a13) {
      if (typeof a13[key] == "number") out += a13[key];
   }
   out13.innerHTML = out;
}
...
// Task 14
// При нажатии b-14 выполняете функцию f14. Функция должна в out-14 выводить нулевые (по индексу)  элементы вложенных массивов в arr. Вывод через пробел.
...
let out = '';
function f14() {
   let outDiv = document.querySelector('.out-14');

   for (let key in arr) {
      console.log(arr[key].length, arr[key][0]);
      out += arr[key][0] + ' ';
   }
...
function f17(arr, block) {
   let outDiv = document.querySelector(block);

   for (let key in arr) {
      for (let key1 in arr[key]) {
         if (key1 == 'age' && arr[key][key1] > 30) out += `${arr[key].name} `;
      }
   }
...
function f19(arr, block) {
   let inn = document.querySelector('.i-19').value;
   let outDiv = document.querySelector(block);
   ...
   if (inn) {
      ...
      for (let key in arr) {
         for (let i = 0; i < arr[key].length; i++) {
            if (arr[key][i].toLowerCase() == inn.toLowerCase()) out = key;
         }
      }

      outDiv.innerHTML = out;
   } else {
      outDiv.innerHTML = 'Введите значение в поле 19';
   }
}
...
function f20(arr, block) {
   ...
   // for (let key in arr) {
   //можно через перебор массива делать, но лучше так как выше сделал
   //через перебор ассоциативного массива
   //    for (let i = 0; i < arr[key].length; i++) {
   //       if (arr[key][i][1] == 1) out += `${arr[key][i][0]} `;
