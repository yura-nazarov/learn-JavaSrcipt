...
function t7() {
   b7_res = b7.filter(item => {
      if (typeof item == 'string' && item.length > 3) return item;
   })
   return b7_res;
}
...
function t9() {
   ...
   // возвратит ассоциативный массив из двух массивов
   // return {
   //    b9_num, b9_string
   // };
}
...
function t10() {
   b10_res = b10.filter(item => {
      if (item.includes(3)) return item;
   })
   return b10_res;
}
